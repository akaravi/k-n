...
REPO="https://raw.githubusercontent.com/akaravi/k-n/main"
MANAGER_URL="$REPO/Karavi-Tunnel.sh"
PY_URL="$REPO/Karavi.py"

BIN="/usr/local/bin/karavi-tunnel"
PY_DST="/opt/karavi/Karavi.py"
...
curl -fsSL "$MANAGER_URL" -o "$tmp_dir/karavi-tunnel" || err "Failed to download manager"

info "Downloading tunnel core..."
curl -fsSL "$PY_URL" -o "$tmp_dir/Karavi.py" || err "Failed to download tunnel core (Karavi.py)"

# sanity check: non-empty files
[[ -s "$tmp_dir/karavi-tunnel" ]] || err "Downloaded manager is empty"
[[ -s "$tmp_dir/Karavi.py" ]] || err "Downloaded core is empty"

install -m 0755 "$tmp_dir/karavi-tunnel" "$BIN"
mkdir -p "$(dirname "$PY_DST")"
install -m 0755 "$tmp_dir/Karavi.py" "$PY_DST"
