...
def auto_pool_size(role: str = "ir") -> int:
    ...
    Can be overridden with env var KARAVI_POOL (positive int).
    """
    # Allow explicit override
    try:
        env_pool = int(os.environ.get("KARAVI_POOL", "0"))
        if env_pool > 0:
            return env_pool
    except Exception:
        pass
...
def main():
    ...
    if choice == "1":
        ...
        try:
            ...
            nofile = -1
        print(f"[AUTO] role=EU nofile={nofile} pool={pool} (override: KARAVI_POOL)")
        ...
        if yn == "y":
            ...
            try:
                ...
                nofile = -1
            print(f"[AUTO] role=IR nofile={nofile} pool={pool} (override: KARAVI_POOL)")
