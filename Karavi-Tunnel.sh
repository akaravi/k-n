...
SCRIPT_FILENAME="Karavi-Tunnel.sh"
SELF_URL="https://raw.githubusercontent.com/akaravi/k-n/main/${SCRIPT_FILENAME}"

PY="/opt/karavi/Karavi.py"
PY_URL="https://raw.githubusercontent.com/akaravi/k-n/main/Karavi.py"

INSTALL_PATH="/usr/local/bin/karavi-tunnel"

BASE="/etc/karavi_manager"
CONF="$BASE/profiles"
MAX=10

HC_SCRIPT="/usr/local/bin/karavi-health-check"
HC_CRON_TAG="# KaraviTunnelHealthCheck"
...
install_script(){
  ...
  echo "[+] Installed. Run: sudo karavi-tunnel" > /dev/tty
}
...
update_script(){
  ...
  if is_installed; then
    ...
    chmod +x "$INSTALL_PATH"
    echo "[+] Updated. Run again: sudo karavi-tunnel" > /dev/tty
  else
...
optimize_server(){
  ...
  if sysctl net.ipv4.tcp_available_congestion_control 2>/dev/null | grep -q bbr; then
    ...
    local conf="/etc/sysctl.d/99-karavi-tunnel.conf"
    cat > "$conf" <<'EOF'
# Karavi Tunnel - network tuning
